DROP VIEW IF EXISTS vw_xr_ranked_companies_by_day CASCADE;
CREATE VIEW vw_xr_ranked_companies_by_day AS
(
	WITH t1 AS
	(
		SELECT
	...
	FROM t2
	ORDER BY time_period_label, time_period DESC, rank ASC, name
);

---

DROP VIEW IF EXISTS vw_xr_ranked_companies_by_week CASCADE;
CREATE VIEW vw_xr_ranked_companies_by_week AS
(
	WITH t1 AS
	(
		SELECT
...
DROP VIEW IF EXISTS vw_xr_ranked_companies_by_month CASCADE;
CREATE VIEW vw_xr_ranked_companies_by_month AS
(
	WITH t1 AS
	(
		SELECT
