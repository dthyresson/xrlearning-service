DROP VIEW IF EXISTS vw_xr_ranked_category_groups_by_day CASCADE;
CREATE VIEW vw_xr_ranked_category_groups_by_day AS
(
	WITH t1 AS
	(
		SELECT
		  ...
		  JOIN articles a on a.feedly_id = c.feedly_id
			JOIN vw_organization_category_groups oc ON oc.uuid = c.uuid
		  GROUP BY 1, 2, 3
	...
	FROM t2
	ORDER BY time_period_label, time_period DESC, rank ASC, category_group
);

---

DROP VIEW IF EXISTS vw_xr_ranked_category_groups_by_week CASCADE;
CREATE VIEW vw_xr_ranked_category_groups_by_week AS
(
	WITH t1 AS
	(
		SELECT
...
DROP VIEW IF EXISTS vw_xr_ranked_category_groups_by_month CASCADE;
CREATE VIEW vw_xr_ranked_category_groups_by_month AS
(
	WITH t1 AS
	(
		SELECT
