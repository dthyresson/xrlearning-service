...
conn.exec(%Q(
              SELECT
                row_to_json(vw_xr_ranked_category_groups_by_#{time_period_label}) as rankings
              FROM vw_xr_ranked_category_groups_by_#{time_period_label}
              WHERE time_period = $1
              ORDER BY rank
              LIMIT 10
  ...
  result.each_with_index do |row, index|
    ...
    fields << {
                "type": "mrkdwn",
                "text": "#{rankings['rank'].to_s.rjust(2)} | *#{rankings['category_group']}* (#{rankings['freq']})",
              }
  end
end
...
title = ":100: *Top Mentioned Company Submarkets in XR"
